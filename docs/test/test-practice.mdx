...
export const Highlight = ({children, color, bg}) => (
    <span
        style={{
            backgroundColor: bg,
            borderRadius: '5px',
...
<Parallax
    animation={{scale: 1, width: '50%'}} replay={true}
    style={{transform: 'scale(0)', margin: '10px auto'}}
>
    <img src='https://img.springlearn.cn/blog/learn_1620980699000.png'/>
</Parallax>
...
通过 <Highlight color="#ffffff" bg="#117eed"> mvn test </Highlight>在每次编译时候，对代码进行测试。以到达可持续集成的目的。这里的关键是在每次服务发布的时候,首先要运行测试用例。
...
<Highlight color="#ffffff" bg="#e6822f"> 主要看注释,我们在调用接口的时候,先通过给外部接口最Mockito代理,然后指定动作的出入参数。</Highlight>
...
<Highlight color="#ffffff" bg="#117eed"> 针对项目中核心流程, 可能并不仅仅是一个接口，而是有依赖关系的多个接口。</Highlight>
...
<OverPack style={{overflow: 'hidden', height: 200}} playScale={0.2}>
    <QueueAnim key="queue"
               leaveReverse
               style={{float:'left', position: 'relative'}}
    >
        <div key='a' style={{fontStyle:'bold',lineHeight: '2em',fontFamily:'Arial, Helvetica, sans-serif;'}}>
...
<Highlight color="#ffffff" bg="#117eed">做到业务流程闭环</Highlight>
...
<OverPack style={{overflow: 'hidden', height: 100}} playScale={0.2}>
    <QueueAnim key="queue"
               leaveReverse
               style={{float:'left', position: 'relative'}}
    >
